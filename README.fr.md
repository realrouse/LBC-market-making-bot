...
- **Logique du bot testée unitairement** — 30 tests unitaires pour `live_bot.py` (`TestCheckSignal` couvre les 11 gardes d'entrée, `TestCheckResolution` couvre les chemins WIN/LOSS/expiration, `TestCloseTrade`, `TestRestoreState`, etc.) ; combiné à la suite de backtest : 58 tests au total, aucun réseau ni credentials requis
