...
- **Partage WebSocket multi-bot** — architecture trois processus : `indicators.py` (ZMQ PUB/REP pour les données de signal) + `feed.py` (connexion WebSocket unique, ZMQ PUB pour les mises à jour du carnet) + un ou plusieurs processus `account_bot.py` (souscrivent au feed et aux indicators, tradent avec des bases SQLite, logs et configs totalement isolés) ; les trois communiquent via des sockets IPC dans `/run/user/$UID/` — isolation par utilisateur Linux enforced par le noyau, aucun conflit de port TCP ; déployés en services systemd utilisateur (`systemctl --user`, sans `sudo`) ; `feed_auto_start: false` dans `config.json` lors de l'utilisation de services gérés ; chaque bot évalue les signaux avec **ses propres** paramètres de stratégie — voir [docs/multi.fr.md](docs/multi.fr.md) pour le guide de décision et la référence d'architecture complète
