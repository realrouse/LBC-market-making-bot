...
- **Suite de 639 tests** — `tests/test_bot.py` couvre les 11 gardes du signal, les chemins de résolution, le calcul des frais, le parsing WebSocket, la page de statut HTML, le hashage htpasswd, la restauration d'état, la logique du filtre horaire, la mise proportionnelle, le stop-loss hebdomadaire et la configuration de découverte des marchés ; `tests/test_backtest.py` couvre le moteur de replay de bout en bout ; `tests/test_multibot.py` couvre `feed.py` et `account_bot.py` avec intégration ZMQ round-trip ; `tests/test_grid_trail.py` couvre la validation des connecteurs, le branchement du stop-loss, le recentrage de la grid et la restauration depuis la DB ; aucun réseau ni credentials requis
