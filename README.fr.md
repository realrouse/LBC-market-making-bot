...
- **Tableau de bord de statut multi-bot** — `tradinebotte-status/generate_status.py` interroge tous les comptes de déploiement via SSH, lit la base de données des heartbeats, et génère une page HTML unique affichant l'état de chaque bot, les versions des services et les détails de payload (PnL, nombre de positions, connectivité WebSocket) ; chemin de sortie par défaut `~/public_html/tradinebottestatus.html`, modifiable via `--out` ou `$TRADINEBOTTE_STATUS_OUT` ; le collecteur de heartbeats (`tradinebotte-status/status_collector.py`) tourne en service systemd utilisateur sur le premier compte de déploiement et purge automatiquement les lignes de plus d'un an
- **Partage WebSocket multi-bot** — architecture trois processus : `indicators.py` (ZMQ PUB/REP pour les données de signal) + `feed.py` (connexion WebSocket unique, ZMQ PUB pour les mises à jour du carnet) + un ou plusieurs processus `account_bot.py` (souscrivent au feed et aux indicators, tradent avec des bases SQLite, logs et configs totalement isolés) ; les trois se déploient à plat sous `~/tradinebotte/` et communiquent via des sockets IPC dans `/run/user/$UID/` — isolation par utilisateur Linux enforced par le noyau, aucun conflit de port TCP ; gérés en services systemd utilisateur (`systemctl --user`, sans `sudo`) ; `feed_auto_start: false` dans `config.json` lors de l'utilisation de services gérés ; chaque bot évalue les signaux avec **ses propres** paramètres de stratégie — voir [docs/multi.fr.md](docs/multi.fr.md) pour le guide de décision et la référence d'architecture complète
