...
- **API exchange modulaire** — tout le code spécifique Polymarket est dans `bot/api_polymarket.py` ; changer d'exchange ne nécessite qu'un nouveau fichier adaptateur et une seule ligne d'import dans `live_bot.py` ; les connecteurs Binance spot (`bot/api_binance.py`), MEXC spot (`bot/api_mexc.py`) et Bitstamp spot (`bot/api_bitstamp.py`) sont inclus, implémentant l'interface identique avec signature HMAC-SHA256, calcul de l'OBI et mode simulation
- **Gestionnaire Binance Simple Earn Flexible** (`bot/earn_manager.py`) — `EarnManager` place les USDT inactifs après une vente (`park_idle()`) et les rachète avant un achat (`ensure_liquid()`) ; découverte automatique du produit et rapport du taux annuel ; mode simulation si les identifiants sont absents ; MEXC Earn non pris en charge (API trop instable)
- **Service d'indicateurs techniques** (`bot/indicators.py`) — étage pipeline ZeroMQ : souscrit au PUB de feed.py (port 5557), calcule RSI, SMA, EMA et volatilité glissante sur la série `best_bid` de chaque token, republie des messages `{"t":"indicators"}` enrichis sur un second socket PUB (port 5559) ; maths en stdlib pur (sans numpy) ; les consommateurs souscrivent au port 5559 pour recevoir les indicateurs en temps réel en parallèle des données de carnet ; nouvelle source `binance_scalping` combinant les streams Binance depth20 + aggTrade pour calculer OBI, EMA, décélération, `spread_bps`, `realized_vol_bps` et TFI en temps réel ; configuration unifiée en 9 flux dans `strategies/indicators/indicators_all.json` (PUB 5559, REP 5561) ; le flux `btc_4h` inclut désormais EMA(50), EMA(200) et ATR(14) pour les consommateurs de la stratégie swing
...
- **Bot de scalping OBI Binance** (`bot/orderbook_bot.py`) v2.1 — connexion aux streams WebSocket depth20 spot et perpétuel de Binance (100 ms), calcul de l'OBI sur les N meilleurs niveaux bid/ask avec lissage EMA ; stratégie SHORT uniquement (un carnet d'ordres dominé par les bids est interprété comme du spoofing annonçant une baisse) ; TP 15 bps, SL 8 bps, durée maximale 3 minutes ; mode simulation d'ordres à cours limité avec identifiants préfixés `sim_` ; snapshots et trades enregistrés dans `live_ob.db` ; configuré via `strategies/scalping/orderbook_btc.json` (`entry_thresh`, `confirm_n`, `tp`, `sl`, `n_levels`)
- **Stratégie swing trading** (`bot/strategy_engines/swing.py`) — moteur `SwingStrategy` qui place des ordres limit BUY sur des niveaux de support et des ordres SELL sur des résistances ; filtre directionnel EMA(200) 4h — les achats sont ignorés quand le prix est sous l'EMA 200 périodes ; stop-loss dynamique ATR(14) ; filtre RSI(14, 4h) de surachat ; souscription au service d'indicateurs partagé (ZMQ SUB) ; persistance SQLite avec restauration des positions au redémarrage ; configurée via `strategies/swing/swing_BTCUSDT.json` (`supports`, `resistances`, `position_size`, `max_positions`, `atr_sl_multiplier`) ; déploiement via `scripts/update_swing.sh`
- **Bot de scalping live Binance** (`bot/scalping_bot.py`) — trois stratégies indépendantes (`candle_momentum`, `meanrev`, `breakout`) sur données OHLCV 1 minute ; les 27 paramètres de `DEFAULTS` sont documentés dans la docstring du module ; backtestable avec `analysis/backtest_scalping.py`
- **Helpers mathématiques de scalping** (`bot/scalping_math.py`) — ATR, bandes de Bollinger, VWAP, z-score de volume et maximum glissant, partagés entre `scalping_bot.py` et `indicators.py`
...
- **Suite de tests** — `tests/test_bot.py` couvre les 11 gardes du signal, les chemins de résolution, le calcul des frais, le parsing WebSocket, la page de statut HTML, le hashage htpasswd, la restauration d'état, la logique du filtre horaire, la mise proportionnelle, le stop-loss hebdomadaire et la configuration de découverte des marchés ; `tests/test_backtest.py` couvre le moteur de replay, le sizing Kelly fractionnel, les ratios Sharpe/Sortino, l'optimisation walk-forward et le filtre de volatilité par jour de semaine ; `tests/test_multibot.py` couvre `feed.py` et `account_bot.py` avec intégration ZMQ round-trip ; `tests/test_grid_trail.py` couvre la validation des connecteurs, le branchement du stop-loss, le recentrage de la grid et la restauration depuis la DB ; `tests/test_earn_manager.py` couvre les flux subscribe/redeem/mode-sim de `EarnManager` ; `tests/test_cycle_strategy.py` couvre le backtest de la stratégie cycle long terme ; `tests/test_scalping_bot.py` couvre les trois stratégies de scalping (`candle_momentum`, `meanrev`, `breakout`) ; couverture de l'adaptateur Bitstamp dans `tests/test_api_cex.py` ; aucun réseau ni credentials requis
