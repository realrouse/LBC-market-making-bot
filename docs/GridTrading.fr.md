...
levels    = [grid_lower + i × grid_step  pour i de 0 à grid_levels-1]
```
...
`win_threshold`, `loss_threshold`, `min_ask_vol` sont propres à la stratégie
threshold Polymarket et n'ont pas de sens pour le grid.
...
ne part — comportement identique à `--simulate`).
...
# ajuster grid_lower, grid_upper, grid_levels, grid_order_size_usdt
```
...
- `grid_order_size_usdt` : prévoir au moins 10 × `grid_step` de capital
  disponible pour éviter un manque de fonds si tous les BUY se remplissent
...
Vérifier dans les logs que `GridStrategy` charge correctement et que les
...
Dépend de la volatilité. Sur BTC/USDT avec un `grid_step` de 1 000 USDT et
une volatilité journalière de ±3 000 USDT, on peut s'attendre à 6–15 cycles
par niveau actif par jour.
