...
- **`bot/live_bot.py` — `VOL_FILTER_WEEKDAY_ONLY`** — optional flag (default `False`) that suspends the volatility filter during the weekend session (Fri 20:00 UTC → Mon 13:30 UTC); the filter was calibrated on weekday data only, so its thresholds may over-reject valid signals during weekends when BTC liquidity and volatility patterns differ; new helper `_in_weekend_session()` encapsulates the boundary logic and is covered by 10 unit tests
