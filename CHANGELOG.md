...
- **Deploys got much faster: one SSH connection is now reused per account instead of one per step.** Every ssh/rsync call in the native deployer (`scripts/deploy_actions.py`) and a dozen ops scripts (`bot_status.sh`, `botctl.sh`, `check_inventory.py`, `cleanup_server.sh`, `setup_data_plane.sh`, `update_claude1.sh`, …) opened a fresh connection with password authentication, measured at roughly 13 seconds of pure handshake overhead each — one deploy makes about 13 of these calls in sequence, so most of a 4–7 minute deploy was connection setup, not real work. `ControlMaster`/`ControlPersist` now let every call after the first multiplex through one already-authenticated channel; a full account deploy dropped to about 14 seconds in testing, and a 12-step, 7-account fleet-wide deploy (with an infra restart) completed in under 5 minutes. Scripts that must reach several distinct accounts in one run (the fleet-wide `bot_status.sh` overview) see no change — a different account is always a new connection.
...
- **CI runs the same pytest suite as the local runner, and now covers the status service.** The GitHub Actions test job invoked `unittest discover` directly and installed only runtime dependencies — so it neither matched the canonical runner nor loaded the isolation `conftest.py`, and it silently omitted the `tradinebotte-status` test suite entirely. It now installs pytest, runs pytest per directory like `scripts/run_tests.sh`, and includes the status suite. `pytest` is declared in `requirements-dev.txt`, and the runner fails with a clear message if it is missing rather than a raw import error.

- **All systemd unit templates now live in one top-level `systemd/` directory.** They were scattered across `tradinebotte-polymarket/scripts/systemd/` (which had become the de-facto shared location — even the indicators and cex-feed units lived under the polymarket subproject), plus `tradinebotte-cex/` and `tradinebotte-status/`. The native deployer no longer needs a per-service directory override: every family and infra template resolves to `systemd/<unit>` from one constant. A new test asserts every template a deploy spec names exists there, so a wrong path fails in CI instead of at a real deploy.

- **The test runner now also exercises the CEX engine fidelity harness.** `scripts/run_tests.sh` ran the Polymarket backtest (`backtest.py --all`) but not `backtest_engine.py`, which drives the real grid/swing strategy engines on replayed klines and reports their drift from the re-implemented backtests. It now runs the harness against a kline dataset when one is present — gated on a `klines`-table check, so it skips cleanly in CI (data DBs are not in git, where the in-memory parity test already guards the engine) and runs locally for anyone with kline data. Non-blocking.

- **The status page groups bots by strategy family again — with an explicit `strategy_type`, not hardcoded names.** After the fleet moved to generated bot_ids (`binance-grid-btcusdt-df6dd9`), the "by family" view grouped every bot under its own unique id, so nothing rolled up and the windowed-PnL rollup silently rendered for nobody — its family set was still the pre-migration strategy names (`live_bot`/`grid_bot`/`swing_bot`/`account_bot`), which now match no live bot. The status page is downstream of the architecture, so it is the page that adapts: each bot's family is derived from a new authoritative `strategy_type` column (in `inventory.toml` and the shared DB — `accumulation`/`grid`/`swing`/`polymarket`, with infra categories for services), computed by one canonical normaliser in `inventory_labels.py`. Grid/swing/polymarket instances now roll up per strategy with restored windowed PnL; accumulation stays a holdings-detail family; the payload/key-metric renderers and the account-card metrics — all of which had also gone dead against generated ids — key on the family too. The two real/sim pages (split on `is_live`) are unchanged, and the real-money bot keeps its dedicated page and LIVE badge. The status page reads the DB with a raw query (it never runs the schema migration), so it falls back to deriving the family from `bot_type` when the column is not populated yet — the code deploy is safe before the DB is migrated. Run `python3 tradinebotte-status/sync_inventory.py` after deploying to populate the column.
...
- **Seven dead systemd templates and scripts left by earlier retirements.** `tradinebotte-cex/scripts/systemd/tradinebotte-accumulation.service` drove `accumulation_bot.py`, an entrypoint that no longer exists (accumulation runs through `live_bot.py`); its sibling `tradinebotte-orderbook.service` and the account-hardcoded `deploy_scalping_claude4.sh` deployed the orderbook bot, disabled as unprofitable since June. `tradinebotte-indicators/scripts/systemd/tradinebotte-indicators.service` and `install_service.sh` + `tradinebotte.service` were the single-bot and old system-unit installers whose install scripts were already removed. `tradinebotte-feed.user.service` had no consumer. The install guide's single-bot systemd section now points at the native deploy engine. Live unit templates and the real-money deploy path are unaffected.
...
- **The two remaining hand-run feed scripts.** `install_feed_service.sh` generated a systemd unit for the shared WebSocket feed, and `start_feed.sh` launched that feed outside systemd; the deploy engine now installs and manages the feed unit natively, and the feed has no supported manual launch path. The install guide's manual-launch and multi-bot update sections went with them — both described the retired per-account bot workflow, whose scripts no longer exist.

- **Two infrastructure services no longer mint a bot identity just by being imported.** The indicators and CEX-feed services resolved their fleet id (`bot_id_<role>`) at module import time, and resolving persists a freshly-generated id to disk — so importing either module (a test collecting it, a REPL, a lint) dropped a stray `bot_id_*` file into the source tree or the working directory, each time a different random id. Both now resolve the id inside the service's async runner, the way the polymarket feed and bot already did; a new import-purity test imports every entrypoint in a clean subprocess and fails if any `bot_id_*` appears. Generated ids are also gitignored now.

- **The indicators install script is gone; its manual runner is now labelled debug-only.** `install_indicators_service.sh` generated a systemd unit for the shared indicators pipeline, superseded by the deploy engine's native `indicators` infra target (same as the feed installer). `start_indicators.sh` is kept — it is the only way to run a single indicator stream in isolation for debugging — but its header, and the `docs/indicators` section that showed it, now say so explicitly; it also learned to find the fleet's `.venv` (it only looked for a legacy `venv/`). The INSTALL and multi-bot docs point their systemd instructions at the deploy engine.

- **The legacy strategy names `account_bot` / `grid_bot` / `swing_bot` in the status page.** The bot-family view still listed these pre-`bot_id` strategy names alongside `live_bot` in its PnL-family set, family ordering, payload-summary and key-metric branches, the Polymarket-detection sets, and their i18n labels. Every live bot now heartbeats under a generated `bot_id` (e.g. `binance-grid-btcusdt-…`), so those three names matched nothing in the fleet and rendered nothing — verified against the shared production database (zero heartbeat/inventory rows) and by a before/after diff of the rendered pages (byte-identical). Removed, along with the now-orphaned `feed=` label the deleted `account_bot` branch was the only user of. `live_bot` and the family-ordering mechanism are kept; the unrelated `idx_*_account_bot` SQL indexes (named for the `(account, bot_name)` columns, not the process) are untouched.

### Deprecated
- **The weekly data-collection pipeline is dormant, not removed.** `start_collector.sh`, `collect_db.sh` and `schedule_collect.sh` are not deployed anywhere and have produced no archive since May 2026, but they remain the tooling that builds the high-resolution datasets used for backtesting, so they are kept for a future collection campaign. Each script and its documentation now says so up front, instead of reading as live instructions.
- **`docs/multi.md` and its French version describe a retired architecture.** They document the shared-feed-plus-per-account-bot layout whose per-account component has been deleted. The shared-feed rationale and the ZeroMQ message protocol are still accurate; the deployment and launch instructions are not. Both files now open with a notice to that effect, pending a rewrite.

- **The test runner no longer leaks trades into the shared production database.** `run_tests.sh` runs the suite with `unittest`, which does not load the pytest `conftest.py` that redirects the status channel to a dead port — so a test firing an accumulation trade pushed a real row into the shared state DB, tagged with the operator's account. The runner now sets that dead-port redirect itself, matching the pytest path; a stray push connects to nothing.

- **`purge_account_state.py` now also purges `bot_trades`.** The shared-DB cleanup deleted a test account's heartbeats and deploys but not its per-fill trade rows, so a test that fired a trade left residue the tool could not remove. It now clears all three tables (guarded so a DB predating `bot_trades` still works), and the dry-run and summary report the trade count.

- **`backtest_swing_dca.py` sweep header no longer breaks on Python 3.10/3.11.** Three f-strings put a literal backslash inside the `{...}` expression (`f"{'tp_pct\\sl_pct':<18}"`), which is a `SyntaxError` before Python 3.12 — so importing the module (the engine parity test does) failed on the CI's 3.10/3.11 interpreters while passing on a newer local one. The label strings are now built outside the f-string.

- **The inventory/deploy tooling now runs on Python 3.10.** `deploy.py`, `transfer_bot.py`, `check_inventory.py`, `inventory_labels.py` and `sync_inventory.py` imported `tomllib` unconditionally, but it is stdlib only from 3.11 — so all of them raised `ModuleNotFoundError` on 3.10. They now fall back to `tomli` (added as a `python_version < "3.11"` requirement), matching the version range the rest of the code supports. The CI test matrix also stops cancelling the other Python versions when one fails (`fail-fast: false`).
...
### Documentation
- **Documentation brought in line with the current module architecture** — 0.89 reorganized the code into a neutral core (`botcore`: strategy interface, connector registry, persistence, base schema) with the Polymarket and CEX bots as peer plugins, but the docs still described the older single-file layout. The README, INSTALL, CONTRIBUTING, the design reference, and the multi-bot / snapshots / grid / how-to guides now describe the neutral core and the flat `pm_*` / `cex_consumer` plugin modules, attribute moved functions (e.g. `save_snapshot`, `check_signal`, `cex_feed_consumer_loop`) to the files that now define them, and drop the retired `bot/` subdirectory paths and the obsolete "edit one import to switch exchange" instruction (the exchange is now chosen by a `connector` name in the strategy config, loaded from the registry). English and French docs were updated together.
- **New module-architecture diagram** — added a companion figure (`docs/architecture_modules.png`, produced by `docs/gen_architecture_diagram.py`) showing the neutral core with the Polymarket and CEX plugins as peers, referenced from the design document. The existing service / ZMQ-topology diagram is unchanged, because the runtime topology did not change — only the internal code layout did.
...
### Internal
- **Post-refactor cleanup** — removed a handful of dead re-exports left by the decoupling (symbols nothing referenced), marked the intentional re-export block so the linter stops flagging it (and a future reader doesn't mistake the re-exports for dead code), refreshed two breadcrumb comments, and corrected a stale docstring in the core that still pointed a strategy implementation at its old location.
- **The exchange-agnostic database tables moved into the neutral core** — the migration-version tracker (`schema_version`) and the generic key/value table (`bot_meta`, whose accessors already live in the core) moved out of the entrypoint's schema into a small neutral base schema in the core, applied before the exchange-specific tables. No data migration: every statement is idempotent, and a freshly-created database and an existing already-migrated one provably converge to the same schema and version (covered by a new test). Behavior is unchanged.
- **The data-source dispatch is now registry-driven instead of hardcoded** — choosing which data-consumer loop to run (Polymarket shared feed, CEX shared feed, or direct WebSocket) was a hardcoded `if/elif/else` in the entrypoint that named each plugin's loop and special-cased the Polymarket connector. It is now a small registry mapping each data source to its plugin run-loop, resolved by a pure, unit-tested selector — so a new strategy family plugs in by adding one registry entry rather than editing the dispatch, and the entrypoint no longer privileges one exchange. Behavior is unchanged (every routing case, including the mismatched-source warning and the direct-WS fallback, is covered by tests); the CEX consumer is still imported lazily so a Polymarket-only bot never loads it.
- **Polymarket data path extracted from the universal entrypoint** — the Polymarket book/market-update path — the order-book dispatch, the snapshot writer, market discovery, and the two data sources (the direct WebSocket lifecycle and the shared-feed consumer) — moved out of the universal entrypoint file into a dedicated Polymarket module (`pm_data`). With this, the entrypoint file no longer contains the Polymarket trading code or data plane; both are grouped in their own plugin modules. The entrypoint re-exports them so callers (including `account_bot`) are unchanged, and every moved function body is byte-for-byte identical.
- **Polymarket threshold strategy + trade logic extracted from the universal entrypoint** — the single-entry threshold signal, the order-entry/resolution/close flow, the stake sizing, and the `ThresholdStrategy` wrapper moved out of the universal entrypoint file into a dedicated Polymarket module (`pm_strategy`), continuing the grouping of the Polymarket plugin. The entrypoint re-exports them so callers are unchanged; the trading logic is byte-for-byte identical (the full signal/Kelly/latency/resolution test suite passes unchanged).
- **Polymarket leaf modules extracted from the universal entrypoint** — the Polymarket per-token market-data type, the signal-rejection counters, and the US trading-calendar/holiday filter moved out of the universal entrypoint file into their own co-located Polymarket modules (`pm_types`, `pm_calendar`), the first step of grouping the Polymarket plugin's code together. The entrypoint re-exports them so callers are unchanged; behavior is identical.
- **The CEX data-consumer loop moved out of the Polymarket entrypoint into the CEX package** — the shared-feed consumer loop and its snapshot writer (used only by the CEX grid/swing bots) lived in the universal entrypoint file alongside the Polymarket code. They now live in the CEX package and are loaded only when a bot is actually configured for the CEX feed, so a Polymarket-only bot never imports them. Behavior is unchanged.
- **The exchange connector is injected into the bot state instead of a module global** — the trading and market-discovery code reached its exchange adapter through a module-level `api` global that the entrypoint rebound at startup. The connector is now loaded by the entrypoint and passed into the bot state, and the code reaches it via `state.connector`, so no module global is mutated and the trading code carries no hard reference to any one exchange. Behavior is unchanged. Importing the entrypoint no longer eagerly loads a connector as a side effect.
- **Neutral persistence helpers moved into the core** — the strategy-agnostic persistence/PnL helpers (capital-base read/write, the shared snapshot-persistence step and its commit-cadence constant, and the cumulative-PnL/equity calculators) moved from the Polymarket entrypoint into `botcore.persistence`; they take a raw connection or a duck-typed state and reference no exchange concept. The entrypoint re-exports them so existing callers are unchanged. Part of consolidating the genuinely shared machinery in the neutral core.
- **The shared bot state no longer hard-codes a Polymarket strategy** — `BotState` used to instantiate the Polymarket threshold strategy directly in its constructor, coupling the shared state object to one exchange's strategy. The active strategy is now passed in by whichever entrypoint builds the state (the Polymarket bots inject the threshold strategy; the grid/swing path still swaps in its own afterwards), so the state object itself names no strategy. Behavior is unchanged. This continues moving the strategy-agnostic machinery toward a neutral core.
- **Connector registry moved into the neutral core** — the connector-load interface (name→module registry + lazy loader) physically lived in the CEX package (`connectors/`) even though it is exchange-agnostic. It now lives in `botcore.connectors`; the old `connectors/` package re-exports from it so `from connectors import load` keeps working for the CEX feed, strategy engines, and tests. The core stays plugin-free — importing `botcore` pulls in no exchange module (loading stays lazy, by name) — and a test asserts that invariant. The bot install also now copies the whole `botcore/` package as a directory instead of a hand-listed file set, so future core modules ship without editing the installer.
- **New neutral core package (`botcore`) owning the strategy interface** — the `Strategy` protocol moved out of the CEX package (`strategy_engines/base.py`) into a new exchange-agnostic `botcore` package that imports nothing from any exchange. `strategy_engines/base.py` re-exports it, so the historical import path keeps working. The Polymarket `ThresholdStrategy` now subclasses `botcore.Strategy` explicitly (previously it only matched the interface structurally), making the protocol's first real conformer; the CEX engines follow in a later step. `botcore/` is shipped flat beside the bot by every install/update/deploy script (like `connectors/`), and a structural test fails if a bot-shipping deploy script forgets it. This is the first slice of an ongoing extraction of the strategy-agnostic core out of the Polymarket entrypoint.
