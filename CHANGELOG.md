...
### Internal
- **Neutral persistence helpers moved into the core** — the strategy-agnostic capital-base read/write helpers (plain SQLite, exchange-independent) moved from the Polymarket entrypoint into `botcore.persistence`; the entrypoint re-exports them so existing callers are unchanged. Part of consolidating the genuinely shared machinery in the neutral core.
