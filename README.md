...
- **639-test suite** — `tests/test_bot.py` covers all 11 signal guards, resolution paths, fee calculation, WebSocket parsing, HTML status page, htpasswd hashing, state restore, hour filter logic, stake scaling, weekly stop-loss, and market discovery config; `tests/test_backtest.py` covers the replay engine end-to-end; `tests/test_multibot.py` covers `feed.py` and `account_bot.py` including ZMQ round-trip integration; `tests/test_grid_trail.py` covers connector validation, stop-loss branching, grid re-centering, and DB restore; no network or credentials required
