...
- **Technical indicator service** (`tradinebotte-indicators/indicators.py`) — ZeroMQ pipeline stage: subscribes to feed.py PUB (port 5557), computes RSI, SMA, EMA, and rolling volatility on the `best_bid` price series of each token, republishes enriched `{"t":"indicators"}` messages on a second PUB socket (port 5559); pure stdlib math (no numpy); consumers subscribe to port 5559 to get live indicator values alongside book data; new `binance_scalping` source combines Binance depth20 + aggTrade WebSocket streams to compute OBI, EMA, deceleration, `spread_bps`, `realized_vol_bps`, and TFI in real time; unified 9-stream config in `tradinebotte-indicators/strategies/indicators_all.json` (PUB 5559, REP 5561); `btc_4h` stream now includes EMA(50), EMA(200), ATR(14) for swing strategy consumers
...
- **Binance OBI scalping bot** (`tradinebotte-cex/orderbook_bot.py`) v2.10 — connects to Binance spot and perpetual depth20 WebSocket streams (100 ms update rate), computes OBI from top-N bid/ask levels with EMA smoothing; long-only since v2.4; successive upgrades: v2.3 TFI filter, v2.5 calibrated TP/SL, v2.7 VWAP gate, v2.9 volume profile gate, v2.10 macro OBI multi-timeframe gate; TP 15 bps, SL 8 bps, max hold 3 minutes; limit-order simulation mode with `sim_`-prefixed order IDs; records snapshots and trades to `live_ob.db`; configured via `tradinebotte-cex/strategies/scalping/orderbook_btc.json` (`entry_thresh`, `confirm_n`, `tp`, `sl`, `n_levels`)
...
- **BTC accumulation bot v1.2** (`tradinebotte-cex/accumulation_bot.py`) — OBI dip-buy with profit-ladder ratchet; adaptive scale-in; state persistence; deploy with `tradinebotte-cex/scripts/deploy_accumulation_claude4.sh`
