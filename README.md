...
- **Multi-bot WebSocket sharing** — three-process architecture: `indicators.py` (ZMQ PUB/REP for signal data) + `feed.py` (single WebSocket connection, ZMQ PUB for book updates) + one or more `account_bot.py` processes (subscribe to feed and indicators, trade with fully isolated SQLite databases, logs, and configs); all three communicate over IPC sockets in `/run/user/$UID/` — kernel-enforced per-user isolation, no TCP port conflicts; deployed as systemd user services (`systemctl --user`, no `sudo`); `feed_auto_start: false` in `config.json` when using managed services; each bot evaluates signals with **its own** strategy parameters (different thresholds, stakes, or hour filters); works across Linux users — see [docs/multi.md](docs/multi.md) for the full decision guide and architecture reference
