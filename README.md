...
- **Multi-bot status dashboard** — `tradinebotte-status/generate_status.py` polls all deployment accounts via SSH, reads the heartbeat database, and writes a single HTML page showing bot health, service versions, and payload details (PnL, position counts, WebSocket connectivity) for every bot; default output path `~/public_html/tradinebottestatus.html`, overridable via `--out` or `$TRADINEBOTTE_STATUS_OUT`; the heartbeat collector (`tradinebotte-status/status_collector.py`) runs as a systemd user service on the first deployment account and prunes rows older than one year automatically
- **Multi-bot WebSocket sharing** — three-process architecture: `indicators.py` (ZMQ PUB/REP for signal data) + `feed.py` (single WebSocket connection, ZMQ PUB for book updates) + one or more `account_bot.py` processes (subscribe to feed and indicators, trade with fully isolated SQLite databases, logs, and configs); all three deploy flat under `~/tradinebotte/` and communicate over IPC sockets in `/run/user/$UID/` — kernel-enforced per-user isolation, no TCP port conflicts; managed as systemd user services (`systemctl --user`, no `sudo`); `feed_auto_start: false` in `config.json` when using managed services; each bot evaluates signals with **its own** strategy parameters (different thresholds, stakes, or hour filters) — see [docs/multi.md](docs/multi.md) for the full decision guide and architecture reference
...
- **Binance Simple Earn Flexible manager** (`tradinebotte-cex/earn_manager.py`) — `EarnManager` parks idle USDT after sell trades (`park_idle()`) and redeems before buy trades (`ensure_liquid()`); automatic product discovery and APR reporting; sim mode when credentials are absent; MEXC Earn not supported (API too unstable)
- **Technical indicator service** (`tradinebotte-indicators/indicators.py`) — ZeroMQ pipeline stage: subscribes to feed.py PUB (IPC by default, TCP port 5559 when `TRADINEBOTTE_PORT_BASE` is set), computes RSI, SMA, EMA, and rolling volatility on the `best_bid` price series of each token, republishes enriched `{"t":"indicators"}` messages on a second PUB socket (IPC by default); pure stdlib math (no numpy); consumers subscribe via IPC (or TCP 5559) to get live indicator values alongside book data; `binance_scalping` source combines Binance depth20 + aggTrade WebSocket streams to compute OBI, EMA, deceleration, `spread_bps`, `realized_vol_bps`, and TFI in real time; unified 9-stream config in `tradinebotte-indicators/strategies/indicators_all.json`; `btc_4h` stream includes EMA(50), EMA(200), ATR(14) for swing strategy consumers; all three Binance WebSocket loops protected by a 120-second recv watchdog (`asyncio.wait_for`, constant `_WS_RECV_TIMEOUT_S`) to prevent indefinite hangs; liquidations published via the public `wss://fstream.binance.com/ws/{symbol}@forceOrder` WebSocket — no API credentials required; full-depth perpetual futures stream (`btc_full_depth_perp`) alongside the spot stream (`btc_full_depth`), with `market` parameter (`"spot"` or `"perp"`), dynamic book trimming via `bid_depth_pct` / `ask_depth_pct`, and correct futures sync validation via `pu`-chaining; optional shared SQLite orderbook database with `_init_depth_db()` / `_write_depth_to_db()` helpers — `orderbook_current` table stores the latest bucketed book, `orderbook_snapshots` table provides a ring-buffer of timestamped snapshots; enabled per stream via `db_path`, `bucket_size_usd` (default 50), `db_write_every_n` (default 60), `history_retention_h` (default 24); DB file created with `0o644` permissions for cross-user read access; writes are off-loaded via `run_in_executor` to keep the async loop unblocked
...
- **Binance OBI scalping bot** (`tradinebotte-cex/orderbook_bot.py`) v2.12 — connects to Binance spot and perpetual depth20 WebSocket streams (100 ms update rate), computes OBI from top-N bid/ask levels with EMA smoothing; long-only since v2.4; successive upgrades: v2.3 TFI filter, v2.5 calibrated TP/SL, v2.7 VWAP gate, v2.9 volume profile gate, v2.10 macro OBI multi-timeframe gate, v2.12 liquidations gate (`liq_gate`, disabled by default); all strategy parameters overridable via JSON; TP 15 bps, SL 8 bps, max hold 3 minutes; limit-order simulation mode with `sim_`-prefixed order IDs; records snapshots and trades to `live_ob.db`; configured via `tradinebotte-cex/strategies/scalping/orderbook_btc.json` (`entry_thresh`, `confirm_n`, `tp`, `sl`, `n_levels`, `liq_gate`, `liq_long_block_usd`)
- **Swing trading strategy** (`tradinebotte-cex/strategy_engines/swing.py`) — `SwingStrategy` engine places limit BUY orders at support levels and SELL orders at resistance levels; EMA(200) 4h directional filter skips buys when price is below the 200-period EMA; ATR(14) dynamic stop-loss; RSI(14, 4h) overbought filter; subscribes to the shared indicators service (ZMQ SUB); SQLite persistence with position restore on restart; configured via `tradinebotte-cex/strategies/swing/swing_BTCUSDT.json` (`supports`, `resistances`, `position_size`, `max_positions`, `atr_sl_multiplier`); deploy with `tradinebotte-cex/scripts/update_swing.sh`
- **Shared math library** (`tradinetools/tradinetools/math.py`) — ATR, Bollinger Bands, VWAP, volume z-score, and rolling max helpers (`sma_last`, `ema_last`, `atr_last`, `bollinger_last`, `vwap_last`, `vol_zscore_last`, `rolling_max_last`); installed as editable package (`pip install -e tradinetools/`); shared across all four sub-services
...
- **JSON strategy files** — signal and capital parameters live in `tradinebotte-polymarket/strategies/polymarket_BTC5M.json`; switch strategies by pointing `"strategy"` in `config.json` to any file; `tradinebotte-polymarket/strategies/polymarket_BTC5M_piste3.json` adds dynamic stake scaling (`bid_alpha`), OBI rejection (`obi_reject_thresh`), and weekly stop-loss (`weekly_stop_loss`); backtest vs original: PnL +85%, MaxDD −28%, Sharpe 3.28 vs 1.97
- **Long-term BTC cycle strategy** — three production-ready configs: `tradinebotte-cex/strategies/longterm/longtermcyclestrategygridV1.json` (5%/25% rebound/tranche, ×24.0, Calmar 0.54), `V2.json` (4%/20%, ×24.2, Calmar 0.54), `V3.json` (halving-relative prudence tiers T1/T2, Calmar 0.75); backtest using `analysis/backtest_cycle_strategy.py` with flags `--top-mm`, `--rebound`, `--drawback`, `--tranche`, `--prudence`, `--compare`; cycle analysis via `analysis/analyze_btc_cycles.py` and `analysis/analyze_cycle_volatility.py`
- **Configurable market timeframe** — `market_tag_id` and `market_window_mins` in the strategy JSON switch between 5-minute and 15-minute BTC Up/Down markets; `tradinebotte-polymarket/strategies/polymarket_BTC15M_piste3.json` ships ready to use; startup log confirms the active tag and window
- **Connector/strategy compatibility check** — `validate()` in `tradinebotte-cex/connectors/__init__.py` raises a `RuntimeError` at startup if the connector lacks methods required by the chosen strategy, listing every missing method; prevents silent runtime failures when swapping connectors
