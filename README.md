...
> **Server admin note:** `python3-venv`, `python3-pip` (and `python3.X-venv` on Ubuntu 22.04+) must be installed once as root before any user runs `install.sh`. See [INSTALL.md — Server admin prerequisites](INSTALL.md#server-admin-prerequisites-debianubuntu).
