...
- **Les déploiements sont beaucoup plus rapides : une seule connexion SSH est désormais réutilisée par compte au lieu d'une par étape.** Chaque appel ssh/rsync du déployeur natif (`scripts/deploy_actions.py`) et d'une douzaine de scripts d'exploitation (`bot_status.sh`, `botctl.sh`, `check_inventory.py`, `cleanup_server.sh`, `setup_data_plane.sh`, `update_claude1.sh`, …) ouvrait une connexion neuve avec authentification par mot de passe, mesurée à environ 13 secondes de pur overhead de handshake à chaque fois — un déploiement en enchaîne environ 13 à la suite, si bien que l'essentiel d'un déploiement de 4 à 7 minutes n'était que de l'établissement de connexion, pas du travail réel. `ControlMaster`/`ControlPersist` laissent maintenant chaque appel après le premier transiter par un canal déjà authentifié ; un déploiement complet de compte est tombé à environ 14 secondes en test, et un déploiement de toute la flotte (12 étapes, 7 comptes, avec redémarrage infra) s'est terminé en moins de 5 minutes. Les scripts qui doivent joindre plusieurs comptes distincts en une seule exécution (la vue d'ensemble `bot_status.sh` sur toute la flotte) ne voient aucun changement — un compte différent est toujours une connexion neuve.
...
- **La page de statut regroupe à nouveau les bots par famille de stratégie — via un `strategy_type` explicite, plus des noms câblés.** Après le passage de la flotte aux bot_id générés (`binance-grid-btcusdt-df6dd9`), la vue « par famille » regroupait chaque bot sous son propre identifiant unique : plus rien ne s'agrégeait, et le cumul de PnL fenêtré ne s'affichait plus pour personne — son ensemble de familles était resté les noms de stratégie d'avant la migration (`live_bot`/`grid_bot`/`swing_bot`/`account_bot`), qui ne correspondent plus à aucun bot actif. La page de statut est en aval de l'architecture, c'est donc elle qui s'adapte : la famille de chaque bot est dérivée d'une nouvelle colonne autoritative `strategy_type` (dans `inventory.toml` et la base partagée — `accumulation`/`grid`/`swing`/`polymarket`, avec des catégories infra pour les services), calculée par un unique normaliseur canonique dans `inventory_labels.py`. Les instances grid/swing/polymarket s'agrègent désormais par stratégie avec le PnL fenêtré restauré ; l'accumulation reste une famille de détail (avoirs) ; les rendus de payload/métrique et les métriques des cartes de compte — tous devenus muets face aux id générés — se basent aussi sur la famille. Les deux pages réel/sim (séparées sur `is_live`) sont inchangées, et le bot en argent réel garde sa page dédiée et son badge LIVE. La page lit la base par requête brute (elle n'applique jamais la migration de schéma) : elle dérive donc la famille depuis `bot_type` tant que la colonne n'est pas peuplée — le déploiement du code est sûr avant la migration de la base. Lancer `python3 tradinebotte-status/sync_inventory.py` après le déploiement pour peupler la colonne.
...
- **Les noms de stratégie hérités `account_bot` / `grid_bot` / `swing_bot` dans la page de statut.** La vue par famille de bots listait encore ces noms antérieurs au `bot_id` aux côtés de `live_bot` dans son ensemble de familles à PnL, l'ordre des familles, les branches de résumé de payload et de métrique, les ensembles de détection Polymarket, et leurs libellés i18n. Chaque bot actif émet désormais son heartbeat sous un `bot_id` généré (p. ex. `binance-grid-btcusdt-…`) ; ces trois noms ne correspondaient donc à rien dans la flotte et ne rendaient rien — vérifié contre la base de données de production partagée (zéro ligne dans heartbeats/inventory) et par un diff avant/après des pages rendues (identiques à l'octet près). Supprimés, avec le libellé `feed=` désormais orphelin dont la branche `account_bot` retirée était le seul usage. `live_bot` et le mécanisme d'ordonnancement des familles sont conservés ; les index SQL `idx_*_account_bot` sans rapport (nommés d'après les colonnes `(account, bot_name)`, pas le processus) sont intacts.
...
- **L'outillage d'inventaire/déploiement tourne désormais sous Python 3.10.** `deploy.py`, `transfer_bot.py`, `check_inventory.py`, `inventory_labels.py` et `sync_inventory.py` importaient `tomllib` sans condition, or il n'est dans la stdlib qu'à partir de 3.11 — tous levaient donc `ModuleNotFoundError` sous 3.10. Ils se rabattent maintenant sur `tomli` (ajouté comme dépendance `python_version < "3.11"`), en cohérence avec la plage de versions supportée par le reste du code. Le matrix de test CI cesse aussi d'annuler les autres versions Python quand l'une échoue (`fail-fast: false`).
