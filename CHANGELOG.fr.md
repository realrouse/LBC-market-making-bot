...
- `docs/multi.md` / `docs/multi.fr.md` — documentation bilingue complète de l'architecture : pourquoi/quand utiliser le multi-bot, diagramme ASCII, référence des composants, protocole de messages (3 types avec tous les champs), variables d'environnement, arborescence, séquence de lancement, monitoring par compte, modes de défaillance (crash feed, crash compte, coupure réseau), ajout d'un troisième compte, tableau comparatif avec le mode autonome, référence de couverture des tests ; lié depuis README, INSTALL, QUICKSTART, feed.py, account_bot.py
