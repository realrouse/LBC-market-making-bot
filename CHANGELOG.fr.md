...
- **`bot/live_bot.py` — `VOL_FILTER_WEEKDAY_ONLY`** — option (défaut `False`) qui suspend le filtre de volatilité pendant la session weekend (ven. 20h00 UTC → lun. 13h30 UTC) ; le filtre a été calibré sur des données de semaine uniquement, ses seuils risquent de bloquer des signaux valides le weekend où les patterns de liquidité et volatilité BTC diffèrent ; nouveau helper `_in_weekend_session()` qui encapsule la logique de détection, couvert par 10 tests unitaires
