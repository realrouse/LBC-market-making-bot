#!/usr/bin/env bash
# deploy_accumulation_claude3.sh — Deploy and (re)start the BTC accumulation bot
#                                  on the standalone test account (index 2 in TEST_USERS).
#
# Runs accumulation_bot.py alongside any existing bot (independent DB/log).
...
SC_IDX="${TEST_STANDALONE_USER_IDX:-2}"
