...
All scripts read the `POLYMARKET_DIR` environment variable to determine
...
```bash
# Default (system-wide, requires root)
...
- Copy `bot/live_bot.py` and `bot/api_polymarket.py` to `<POLYMARKET_DIR>/`
- Copy `strategies/*.json` to `<POLYMARKET_DIR>/strategies/`
- Create a virtualenv at `<POLYMARKET_DIR>/venv/`
- Install Python dependencies into the virtualenv
- Generate `<POLYMARKET_DIR>/run.sh` (wrapper with `POLYMARKET_DIR` pre-set)
- Verify bot syntax
...
it. The `.htpasswd` file is stored at `POLYMARKET_DIR/.webstatus_htpasswd`
...
also supports. Use the `POLYMARKET_DIR/.webstatus_htpasswd` path.
